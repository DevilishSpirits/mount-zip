...
# Copyright 2026 The Mount-Zip Authors.
#
...
def get_current_version():
    with open('mount-zip.cc', 'r') as f:
        for line in f:
            match = re.search(r'#define PROGRAM_VERSION "(.*)"', line)
            if match:
                return match.group(1)
    raise ValueError('Could not find PROGRAM_VERSION in mount-zip.cc')
...
def update_version(version, date=None):
    ...
    with open('mount-zip.cc', 'w') as f:
        f.write(content)

    # Update README.md
    with open('README.md', 'r') as f:
        content = f.read()
    content = re.sub(r'footer: mount-zip .*', f'footer: mount-zip {version}',
                     content)
    if date:
...
def main():
    ...
    run([
        'git', 'add', 'mount-zip.cc', 'mount-zip.1', 'README.md',
        'DEVELOPMENT.md'
    ])
